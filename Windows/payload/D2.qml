...
{
  //------------------------------------------------------------------------------------------------------------------
  // STANDALONE PERFORMANCE DISPLAY — Traktor 4.5.0.7 compatibility launcher
  //------------------------------------------------------------------------------------------------------------------

  property var standaloneDisplayObject: null
  property var standaloneDisplayComponent: null

  function finishStandaloneDisplayLaunch() {
    if (!standaloneDisplayComponent)
      return

    if (standaloneDisplayComponent.status === Component.Ready) {
      if (!standaloneDisplayObject) {
        standaloneDisplayObject = standaloneDisplayComponent.createObject(null)
        if (standaloneDisplayObject)
          console.log("[StandaloneDisplay4507] Native display CREATED")
        else {
          console.log("[StandaloneDisplay4507] createObject FAILED")
          console.log(standaloneDisplayComponent.errorString())
        }
      }
    } else if (standaloneDisplayComponent.status === Component.Error) {
      console.log("[StandaloneDisplay4507] COMPONENT ERROR")
      console.log(standaloneDisplayComponent.errorString())
    }
  }

  function launchStandaloneDisplay() {
    if (standaloneDisplayObject || standaloneDisplayComponent)
      return

    console.log("[StandaloneDisplay4507] launcher starting")
    standaloneDisplayComponent = Qt.createComponent(Qt.resolvedUrl("NativeDisplayWindow.qml"))
    if (standaloneDisplayComponent.status === Component.Loading)
      standaloneDisplayComponent.statusChanged.connect(finishStandaloneDisplayLaunch)
    else
      finishStandaloneDisplayLaunch()
  }
